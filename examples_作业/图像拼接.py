...
# 检测兴趣点并计算描述子
keyp1, describe1 = sift.detectAndCompute(img1, None)
keyp2, describe2 = sift.detectAndCompute(img2, None)

# 绘制特征点
output_image1 = cv.drawKeypoints(img1, keyp1, None)
output_image2 = cv.drawKeypoints(img2, keyp2, None)
# 显示结果图像
cv.imshow('SIFT Features', output_image1)
cv.imshow('SIFT Features', output_image2)
cv.waitKey(0)
...
# 可视化特征匹配结果,并保存
pic3 = cv.drawMatches(img1=img1, keypoints1=keyp1, img2=img2, keypoints2=keyp2, matches1to2=good, outImg=None)
filename = os.path.join(os.getcwd(), "pingjie.png")
cv.imwrite(filename, pic3)


# RANSAC算法计算单应性矩阵
if len(good) > MIN:
    src_pts = np.float32([keyp1[m.queryIdx].pt for m in good]).reshape(-1, 1, 2)
    tge_pts = np.float32([keyp2[m.trainIdx].pt for m in good]).reshape(-1, 1, 2)
